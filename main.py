...
trainset = torchvision.datasets.CIFAR10(root='./data/tiny-imagenet-200/train' transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=False, num_workers=2)

testset = torchvision.datasets.CIFAR10(root='./data/tiny-imagenet-200/test', train=False, download=True, transform=transform_test)
...
def train(epoch, net, count):
    print("trian network", net_layer[count])
    print('\nEpoch: %d' % epoch)
    net.train()
    train_loss = 0
    correct = 0
...
def test(epoch, net):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt10.ta')
        best_acc = acc
...
torch.save(state, './checkpoint/check_train_10')